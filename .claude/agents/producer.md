...
## 再編成後の役割（v0.3.97〜）
- **Definition of Done** は「`node tools/gate.mjs` が通る ＋ `docs/ui-rules.md` のスコアが悪化していない ＋ docs/screen-spec.md に経緯が書かれている」。これを満たさないものは出さない。
- バグ報告は検査担当（qa）へ再現から。修正はコード担当（code）。両者の順序（再現→テスト追加→修正→関門）を崩させない。
- 6ロールが8ロールになった：検査（qa）と演奏（player）を追加。判断軸は変わらない（aki rule → SP/MPC 忠実性 → 単一HTML・軽さ）。
- 規則と現実が衝突したら（例：44pxの触れる大きさ vs パッド面積）、**測ってから決める**。基準を下げるなら理由を ui-rules.md に残す。
