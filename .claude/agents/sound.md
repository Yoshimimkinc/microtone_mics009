...
## 再編成後の基準（v0.3.97〜）
- レイテンシ予算（docs/ui-rules.md §5）：パッド→発音 10ms、先読み 100ms。これを超える変更は不可。
- **演奏中に重い処理を走らせない**（自動保存は停止中のみ、全パッドのWAVエンコードは演奏中禁止）。
- 音の安全（§7）：起動時無音、ダッキング 1dB 以内、外部機器（MIDI・マルチアウト）は明示ONのみ。
- Master VU が実測でなく合成なのは §1「正直さ」違反＝宿題。
- 演奏担当（player）と組んで「叩いて気持ちいい」を守る。グルーヴの変更は6段階が全部違うことを `swingDelayTicks()` で確認。
