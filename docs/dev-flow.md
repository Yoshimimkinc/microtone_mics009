...
| `src/js/` | 30枚：番号付き（boot / engine-* / plock-undo / voice / presets / ui-copy / ui-wave / ui-window / transport / step-strip / menu / edit-modal / chop / autosave / layout / pattern-master / sampling / save-load / midi / share-export / skin-version-splash）＋ 責務別ディレクトリ `ui/`（status / pads-view / performance-view / seq-view）`data/`（pads / patterns）。新しい部品は責務別ディレクトリへ（Phase 2、`docs/modularization-log.md`） |
