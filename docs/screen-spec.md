...
## 86. 【仕様確定・実装待ち】SEQ/パッドのドラッグ&ドロップ＝コピー（衝突ゼロ原則）
基本＝ドラッグでコピー。既にドラッグが意味を持つ場所は「長押し300ms→ドラッグ」で開始（時間分離＝既存操作と衝突しない）。
| 要素 | 既存操作(維持) | コピー操作 |
|---|---|---|
| パターンA-D | タップ=切替/予約 | そのままドラッグ→別パターンへ（8px閾値） |
| BAR1-4 | タップ=小節切替 | そのままドラッグ→別BARへ |
| ノート(SEQセル) | タップ=ON/OFF | 長押し→ドラッグ→別セルへ。値+P-LOCK全パラメータ引き継ぎ。別トラック行も可 |
| stepStrip | タップ=ON/OFF、P-LOCK中横ドラッグ=値(不可侵) | 長押し→ドラッグのみ |
| パッド(EDIT中) | タップ=ひらく | ドラッグ=コピー / 長押しドラッグ=入替（現行の反転＝基本コピー原則に統一） |
| パッド(通常) | タップ=発音 | 導入しない（誤爆防止） |
共通: ドロップ先droptgtハイライト / 成立時pushUndo+「↩で戻せる」/ 異種間ドロップ無効。
安定後、COPYボタン(元→先タップ)の廃止を検討（入口重複の排除）。
実装時の要注意: stepStripのP-LOCK値ドラッグ(3312-)、editDrag(2460-)、stepDrag(1913,2469-)、copyTap/doCopy(2760-)との統合。
