...
## 114. 選択中を見えるように／パッド右下にキー表示（v0.3.102）
- perf の A-D / 1-4 の**選択中**：薄い色味フィル（4.26:1、光の盤で埋もれる）→ **ベタ塗り＋濃い文字**。SEQ の `.patbtn.on` と同じ作法。ui-audit の「値の不足」から消えた。
- **パッド右下にPCキー**：ASSIGNで変えたキーが優先、無ければ既定の 1-8 / Q-I。左下の番号 `.n` と同じ 10px・同色。
  `paintPadStates()` で毎回描く（安い）。ヒット時・P-LOCKページ中の色/減光も `.n` と同じ規則に載せた。
- 監査で PHRASE 1〜3 の名前が 1.92:1 と出るのは、新しい既定プロジェクトで**パッド1〜3がミュート**（`opacity:.4`）だから＝状態表示。

## 115. 波形の再生位置カーソルを窓でも・再生中も（v0.3.103）
モーダルにあった再生位置カーソル（`#pePlayhead`）を、**情報窓の SAMPLE ページでも、手叩きでもシーケンサー再生中でも**出す。
- 実体は2px幅の div を rAF で動かすだけ（canvas は描き直さない）。負荷は 1フレーム 0.1ms 未満。
  ループは常に1本＝再トリガで前のを止めるので、再生中にステップが来ても増えない
- 幅は開始時に1回だけ測る（毎フレーム `clientWidth` を読むとレイアウト計算を強制する）
- 出す条件：`peWaveVisible()`（波形が画面に出ている）かつ 対象が `peTarget`。モーダル限定・手叩き限定は撤廃
- 音のスケジュールは100ms先読みで別スレッド＝このループが詰まっても発音には影響しない

check.mjs：SAMPLEで叩くと出る／再生中も出る／波形が無いページでは出ない、を追加。

## 116. 入口の合言葉（v0.3.104）
「MICS009の入口は合言葉。だれでも入れないように」。
- スプラッシュに合言葉欄（`#splashPass`）。通るまで「TAP TO START」を出さず、画面をタップしても始まらない
- **合言葉そのものはソースに置かない**。SHA-256 のハッシュだけを持ち、入力をハッシュして比較
  （`crypto.subtle`。http の非セキュア文脈向けに純JSの SHA-256 も同梱し、一致を検査している）
- 通ったら端末に覚える（`localStorage` `mics009.pass`）＝メンバーは1回だけ。RESET では消さない
- 違うと赤字で「合言葉が違います」＋揺れる。Enter でも ENTER ボタンでも
- **これは「うっかり入らない」ための鍵**。単一HTMLなのでソースを読める人には本気の防御にならない。
  本当に閉じるならサーバ側（WordPress のページパスワード／Basic認証）。ここは仕様として明記
...
## 119. 下段のエフェクトがスクロールしないと見えない（v0.3.107）
低い画面（実機の Safari バー込み）では `#viewPads` が縦スクロールするので、v0.3.106 で足した下段の
MUTE / DELAY / REVERB がフォールドの下に隠れていた。
窓を上に貼り付けた（§117）のと対で、**下段を `position:sticky; bottom:0` で下に貼り付ける**。
背景 `var(--bg)` を敷き、下を通るパッドが透けないようにした。パッドは窓と下段の間でスクロールする。
check.mjs：390×664 で下段がスクロール無しで見える（コンテナ内・高さ44px以上）。

## 120. 画面の余り：窓の空白44pxを FX の統合で埋める（v0.3.108）
「全体的にまた画面が余ってない？」。測ると 390 幅で、情報窓の **MAIN と FX ページに 44px の空白**
（2行ページ TONE/ASSIGN に合わせた予約 78px の余り）。SAMPLE/TONE/ASSIGN は 0〜4px。

削るのではなく中身で埋めた：**FX（DELAY / REVERB）を MAIN の2行目へ統合**し、ページを 5→4 に。
- MAIN = PITCH / SCALE / LEVEL ＋ DELAY / REVERB（スマホで2行、PCで1行）＝ドラムマシンで一番触る5つが1ページ
- ソフトキーは4つになり、1つあたりの幅が 65→84px（390幅）
- 窓の高さは変えていない（全ページ 203px のまま。§103 の不変性維持）。MAIN の空白 44→4px

残りの余り（数値だけ記録）：ヘッダ `.top` が 50px のボタンに対して 87px（ロゴ行が別段）。
ロゴをスマホで畳めば約35px＝パッド1枚 +9px だが、ブランドの露出なのでプロデューサー判断。

## 121. AKAI スキンでメニューの文字が読めない（v0.3.109）
「SKINでAKAIを選ぶと、設定の文字がめちゃくちゃ見にくい」。
原因：モーダル／トースト／本体の背景が `linear-gradient(テーマのパネル色 → 固定の暗色 #161a1f)`。
暗いテーマではパネルも暗いので気づかないが、AKAI（明るい盤）では**下半分が暗いのに文字はテーマの墨色**。
実測（390幅・メニュー）：**34項目中25項目が 4.5:1 未満、最悪 1.04:1**（SP CH FILTER）。

直し方：固定色をテーマ側のトークンにした。
- `--modal-deep`：グラデーションの下端色（暗いテーマ＝#161a1f のまま、AKAI＝パネル色）
- `--well` / `--well-ink`：窪み（セグ・readout・入力欄・トグルの溝）の受け皿と、その上の文字色。
  暗い受け皿はどのテーマでも暗いままにし、**中の文字は必ず明るい色**に（SP/TR ボタンが 2:1 だった）
- AKAI の `--dim` を #5a5546 → #4a4639（明るい盤で 3.6〜4.4 だった薄い文字を 4.5 以上へ）

結果：AKAI のメニュー **34項目すべて 4.5:1 以上**（0件）。SP/TR スキンは変化なし（見出し2〜3項目が 4.2〜4.4 で従来どおり。ラベルの基準 3:1 は満たす）。
教訓：**テーマ変数の隣に固定色を置かない**。明るいテーマを足した時点で破綻する。

## 122. キーボード→発音のレイテンシ（v0.3.110）
「キーボードに対してレイテンシーが気になる」。
まず測った（Playwright・1280幅）：keydown→発音予約の JS は **中央値 0.3ms**（初回だけ 4〜5ms＝ピッチ焼き込み）。JS は原因でない。
原因は**音声経路の固定遅延**（OfflineAudioContext でインパルスを通して実測）：

| 段 | 遅延 | 備考 |
|---|---|---|
| レンダ量子 | 2.9ms | 128 frames @44.1k。予約→開始の最大待ち。減らせない |
| テープ（wow/flutter の遅延線） | **3.0ms** | 基底 3ms。揺れ幅は合計 0.48ms しかない |
| 標準コンプ `DynamicsCompressorNode` | **6.0ms**（264 frames） | 仕様の先読み。ライブ叩きは並列ドライ 35% で誤魔化していたが、**一発目はドライが立ち上がる前（25ms時定数）＝素通しの6ms** |
| 合計（アプリ内部） | **11.9ms** | 規則 §5 の予算 10ms を超えていた |

直し方：
- テープ基底 3→**1ms**（揺れ幅 0.48ms より大きければ足りる。固定遅延は音に出ない）
- コンプを**先読みなしの AudioWorklet `sp-comp`** に差し替え（0 frames）。Web Audio 仕様と同じ静特性（threshold/knee/ratio のソフトニー）と自動メイクアップ `(1/fullRangeGain)^0.6` を再現。
  アタックは実素材（ドラムパッドの2小節）で標準ノードと RMS を比べて **2ms** に決めた（8ms だと頭が素通りして +2.3dB、2ms で +0.4dB）。
  非対応ブラウザは標準ノードのまま（その時だけ従来の並列ドライが効く）
- 先読みなしなので頭の一発が +3dB ほど通る。サチュレータ（WaveShaper）は定義域の外を端に張り付ける＝硬いクリップになるため、
  定義域を ±2 に広げた（手前に 0.5 のゲイン、|x|≤1 の写像は従来と同一）。1〜2 は tanh で滑らかに丸まる
- メニュー Master に **Latency「内部 / 出力」**を出した。出力（`AC.outputLatency`）が 60ms を超えたら「Bluetooth?」。
  アプリで縮められるのは内部だけなので、残りがどこにあるかを本人が見られるようにする

結果：内部 **11.9 → 3.9ms**。RMS差 +0.39dB（±1dB を §13 で常時検査）。JS中央値 0.3ms。
教訓：**「速い」は測ってから言う**。JS を疑ってから測ったら、遅延は全部ノードの中にあった。

## 123. perf の P-LOCK 行から「P-LOCK」の縦書きラベルを撤去（v0.3.110）
「文言は不要です」。PITCH/LEVEL/FILTER/DELAY/REVERB/REC/NUDGE の並び自体が何の行かを語っているので、見出しは重複（§113 の方針と同じ）。
`.perf-plk-label` の span と CSS を1件ずつ削除。空いた幅はボタン7つに配られる。

## 124. スマホでパッドの下2行が切れる／ブランド名の扱い（v0.3.111）
「スマホで下が切れますね」「パットが切れるんだな」。実測（390×664＝実機Safariのバー込み）：パッド 447〜695 に対し下段（sticky）が 599〜。
下2行が下段の裏に隠れ、スクロールしないと叩けなかった（§107 の「この中だけスクロール」がドラムマシンとして駄目だった）。

縦の予算（664）を1行ずつ数えて詰めた：

| 項目 | 前 | 後 | 差 |
|---|---|---|---|
| ヘッダ（ロゴ行＋トランスポート） | 87 | 59 | −28（ロゴを PADS/SEQ の行へ移動＝`relocateLogo`） |
| PADS/SEQ 行の余白 | 4/8 | 2/4 | −6 |
| 窓 → strip → パッドの余白 | margin 6＋gap 8 ×2 | gap 6 のみ | −16 |
| パッドの下限 | 56 | **44**（規則 §2 の演奏系タップ値） | −48 |
| 下段の上余白・本体の下余白 | 4+4 / 12 | 0+2 / 8 | −10 |

結果：390×664 で **16枚が下段の上に収まる**（パッド 46px、余り 6px）。375×667 も可。390×600 のような極端に低い画面だけ従来どおりスクロール。
844 では余った分がそのままパッドに戻る（96px）。

ブランド名：「microtone のブランド名も要らない」→「でもスクショで訴求できないのは痛い」。
...
## 126. 実機 iPhone 16e（v0.3.112）：16枚は収まったが本体の下に黒い余り 87px（v0.3.113）
「下がずいぶん余ってますね。あまりなく使って欲しい」。スクショを CSS px に換算（2000px ≒ 844）：本体の上端 53（=余白6＋安全域47）、下端 757。
844−757＝**87** ＝ **47**（ステータスバー分）＋ **40**（安全域34＋余白6）。
つまり body の高さは 797＝844−47 で固められていた。iOS の全画面では innerHeight/visualViewport/clientHeight がすべて
ステータスバー分を引いた値を返すのに、レイアウトの原点は画面の上端（上端の位置 53 が証拠）。§125 の最大値取りでは足りなかった。

直し方：iOS かつ全画面（`fullscreenElement` / `webkitFullscreenElement` / `navigator.standalone`）のときだけ、
縦持ちの `screen.height`（画面の実寸、CSS px）と比べて大きい方を採る。Android は全画面で innerHeight が実寸なので触らない
（screen.height はナビバー込みで大きすぎることがあり、逆に下段が切れる）。
残る余白は安全域34（ホームバーの上には触れる物を置かない）＋6px のみ。浮いた47px は flex でパッドへ戻る（4行で約12px ずつ）。
検証は実機のみ（§125 と同じ理由）。

## 127. FAT BASS：「この機材は音が太い」＋ WaveShaper の隠れ遅延（v0.3.114）
「低音を気持ち持ち上げて、太い印象に」「低倍音を発生？レイテンシーとのバーターか」。
バーターにはならない：倍音は WaveShaper（サンプル毎の写像）、量感はローシェルフ（最小位相 biquad）で、どちらも先頭遅延ゼロ。
遅延が出るのはリニアフェーズ EQ／ルックアヘッド型だけで、今回は使わない。

**やったこと**
- spOut 直後にローシェルフ **+2dB @100Hz**（コンプの手前＝低域でコンプが呼吸する）
- 既存の低域経路（150Hz LP → 非対称 waveshaper＝偶数倍音）の混ぜ量 0.12 → **0.18**。スマホの小さいスピーカーは 150Hz 以下を出せないので、2次倍音（100〜300Hz）が「低音がある」印象を作る
- メニュー Sound に **FAT BASS**（既定 ON）。OFF＝v0.3.113 までの音。耳で決められるように A/B を残す

実測（ドラムパッド2小節・出力段まるごとの複製）：
...
| 400〜3kHz | −0.2dB |
| 全体 RMS | +0.6dB |
| ピーク | 不変（コンプと finalClip が受ける） |
| 先頭遅延 | 0 frames（shelf・低域経路とも） |

**ついでに見つけた隠れ遅延**：出力段まるごとの複製にインパルスを通したら **172 frames**。テープ 44 frames のほかに、
WaveShaper の `oversample:"2x"`（Chrome の再標本化 FIR）がサチュレータで **54 frames**、finalClip で **64 frames**＝合計 2.7ms。
§122 の「内部 3.9ms」はこれを数えていなかった（実際は 6.6ms）。2x と none の音の差は 10kHz 以上で 0.6dB（26kHz イメージングを故意に出す
...
## 129. 開発をモジュール化し、公開はガッチャンコにする（v0.3.116）
「動くプログラムは一つだけど、GitHubの中なので開発をより効率化したい」。
効率を落としていたのは1ファイルそのものではなく、**検証と配布の往復**だった。
iOS 固有の不具合（§125 §126 §128）は手元で再現できず、直すたびに main へ出して実機で見る往復が要った。

入れたもの：
1. **ブランチのプレビュー**（`.github/workflows/pages.yml`）… push すると `/preview/<branch>/` に出る。
   本番と同じ origin なので合言葉は共有。一覧は `/preview/`。ブランチ削除で自動的に消える
2. **CI の関門**（`.github/workflows/gate.yml`）… `SKIP_GATE` で飛ばせる手元のフックと違い、
   PR が赤なら merge できない。検査スクショは Artifacts に14日残る
3. **src/ への分割**（45部品）と `tools/build.mjs`。**生成物は1バイトも変えずに**分割できたことを
   `cmp` で確認済み。バージョンは `version.json` だけが正になり、3箇所の手動同期が消えた
4. 判断待ちを **Issues**（`role:` ラベル）へ。会話が切れても残る
5. 出したら **タグ**（`v0.3.x`）

ついでに直したもの：`.githooks/pre-push` がタグ push / ref 削除でも関門を回そうとして
「本体が居ない」で止まっていた（このセッションで実際に踏んだ）。中身を変えない push は回さない。

教訓：**1ファイルは配布の形であって、作業の形ではない。** 分けるときは「出力が1バイトも変わらない」を
先に証明してから分ける。そうすれば分割そのものは無リスクになる。

## 130. CI の初回が炙り出した「この環境でしか動かない前提」（v0.3.117）
§129 で GitHub 側にも関門を置いた初回、**手元では全項目パスなのに CI では2件落ちた**。CI を入れた目的そのもの。

| 落ちたもの | 原因 | 直し方 |
|---|---|---|
| `static-check.mjs` | `import ts from '/opt/node22/.../typescript.js'` と**絶対パス決め打ち**。CI には無い | `TS_PATH` → `node_modules` → `/opt` の順に解決。無ければ理由を出して止まる（`pw.mjs` と同じ作法） |
| `check.mjs` | 起動タップで**全画面に入ったまま** `setViewportSize`。Chromium 153 は「先に通常状態へ戻せ」と拒否する（手元の 1194 では通っていた） | 変える前に `exitFullscreen()` して抜ける（`unFull()`） |

あわせて CI の版を手元と揃えた（`playwright@1.56.1` / `typescript@5`）。
**検査の道具に「この環境なら在る」前提を書かない。** 書いた瞬間、その検査は自分の机の上でしか意味を持たなくなる。

## 131. プレビューは砂場にする：自動保存を本番と分ける（v0.3.118）
§129 のプレビューは本番と**同じ origin**（`…/preview/<枝>/`）に置いた。合言葉（localStorage）を
共有できるのが狙いだったが、同じ理由で **IndexedDB も共有**になる。自動保存は origin 単位なので、
プレビューで触った内容がそのまま本番のセッションを上書きし、プレビューの RESET が本番の自動保存を消す。

直し方：保存先の名前をパスから決める。`/preview/<枝>/` で開いていれば `mics009-preview-<枝>`、
本番は従来どおり `mics009`。合言葉は共有のまま（入り直さずに済む利点は残す）。
`previewName()` / `autosaveDbName()` は引数でパスを渡せる純関数にして、`check.mjs` §15 で検査する。

あわせてスプラッシュに **`PREVIEW · <枝名>`** の橙色バッジを出す（プレビューのときだけJSが挿す）。
「直したのに本番に反映されない」「本番のつもりで作り込んでいた」を防ぐ。

教訓：**同一オリジンの利点と欠点は同じ根から出る。** 共有したいもの（合言葉）と
共有したくないもの（作業中のデータ）を、名前空間で分けて選ぶ。

## 132. P-LOCK は「効果のON/OFF」＋「つまみ位置の記憶」（v0.3.119→v0.3.121）
「エフェクトP-LOCKを解除してもう一度適用すると数値はデフォルトに戻ってしまう。さっきの設定をキープしてほしい」。

**v0.3.119 で読み違えた。** 「解除しても値が残る」と解釈して解放時の巻き戻しを撤去したが、求められていたのは
「**解除＝掛ける前の状態に戻る（効果オフ）／もう一度押す＝最後にいじった値が復活**」。
エフェクトのON/OFFスイッチにつまみ位置の記憶が付いている、という機材の作法。

| 操作 | v0.3.118 | v0.3.119（誤） | **v0.3.121** |
|---|---|---|---|
| DELAY を選んでパッドを 70% に | 0.70 | 0.70 | 0.70 |
| 解除 | 0（戻る） | 0.70（残る） | **0（戻る＝オフ）** |
| もう一度押す | **0（記憶なし）** | 0.70 | **0.70（復活）** |
| スマホの DELAY/REVERB | 戻らない（PCと違う） | 同じ | **PCと同じ** |

実装：`perfLast[param]` に解放時の16パッドの値を控え、次に選んだとき掛ける前の値（`perfSnap`）を取ってから復活させる。
スマホの DELAY/REVERB も `perfLockApply` を通す＝PC と同じ動き。REC ON のときは従来どおり解放しても基本値を保持。
「戻る先」は PLOCKS の既定値ではなく**そのP-LOCKを掛ける前の値**（LEVEL で全パッドが −4dB に揃ってしまわないため）。
記憶はセッション内（リロードで消える）。検査 `check.mjs` §16（解除→0、再適用→0.7、再解除→0 を PC/スマホで）。

教訓：**「キープ」が何を指すかは実機の作法で決まる。** 言葉だけで解釈せず、表にして確認してから直す。


## 133. スマホで P-LOCK の送り量が見えなかった（v0.3.122）
「スマホだとパッドのエフェクト送り量が見えない」。msbar の DELAY/REVERB を押してパッドを左右にドラッグすると
値は変わり音も変わるのに、**塗り（`.lockfill`）も数字（`.lockv`）も出なかった**。

原因：塗りを出す条件が `body.perf`（PCの演奏レイアウト）だけだった。`renderPerfFills` の描画条件と、
CSS を効かせる `body.perf-plock` の付与の両方が `body.perf` を見ていた。スマホは縦持ちで `perf` にならないので、
ドラッグで `perfFillPad` が幅を書いても CSS が `display:none` のまま。

直し：条件を `plockFillsOn()` に一本化。**PC は `body.perf` のとき、スマホは msbar の DELAY/REVERB ボタンが
見えているとき**（`getComputedStyle` で判定＝CSS の 767px 境界と自動で揃う）。`updatePerf`（resize）でも
追従させ、スマホ↔PC で幅が変わっても塗りの条件が食い違わない。
検査 `check.mjs` §16 に「選ぶ→70%→塗り block・幅>60%・数字あり／解除→`perf-plock` が消える」を PC/スマホで追加。

教訓：**同じ機能を2つの画面に載せたら、見た目の条件も2つ分にする。** 値の経路（`perfLockApply`）は
v0.3.121 で共通化したが、表示の条件が古いままだった。

## 134. スマホの下段を6つに：MUTE + PITCH/LEVEL/FILTER/DELAY/REVERB（v0.3.123）
「MUTE DELAY REVERB のボタンを 2/3 の大きさにしてフィルターと他のエフェクトも入れられるか」。

**入れた。PC の演奏ページと同じ5つ**（PITCH / LEVEL / FILTER / DELAY / REVERB）＋ MUTE の6つを1行に。
v0.3.121 で値の経路（`perfLockApply`）、v0.3.122 で塗りの条件（`plockFillsOn`）を PC/スマホで共通化して
あったので、ボタンを足すだけで動く（配線は `.msbar .msbtn.fx` の一括）。

| 幅 | 1つの幅 | 高さ | 行数 |
|---|---|---|---|
| 375（SE/mini） | 55px | 44px | 1 |
| 390（14/15/16） | 57px | 44px | 1 |
| 430（Pro Max） | 63px | 44px | 1 |

以前は3つで 118px だったので約半分（頼まれた 2/3 より詰めた）。高さ 44px は守る（演奏系タップの規則値 §2）。
幅を稼ぐため：点（`.d`）を出さない・字を 10px・字間 .02em・左右余白 4px・ボタン間 4px。
文字は `scrollWidth>clientWidth` で溢れなしを検査。パッド16枚が下段の上に収まる高さ予算（§124）は変わらない。

FILTER はドラッグで cutoff。`filter=off` のパッドは LP に切り替わって音が通る（演奏ページと同じ `perfDrag`）。
...
## 136. スマホの strip（‹名前› LOAD SMPL）を撤去（v0.3.125、Issue #145）
「スマホで LOAD / SMPL ボタンを使わない。EDIT を押すと両方とも手に入るなら検討の余地がある」→ GO。

#145 の元案は「SAMPLE ページの START/END 数値欄を潰して LOAD/SMPL を置く」だったが、
EDIT モーダルには **⤓ LOAD / ● SMPL が既にある**（v0.3.120 の名前行の下）。
なので**スマホの `.strip` を消すだけ**で成立し、SAMPLE ページは触らない。

| | 前 | 後 |
|---|---|---|
| 390×844 パッド1枚の高さ | 92px | **109px** |
| 375×667（SE/mini） | 47px | **64px** |

- `#samp`（ファイル入力）は strip の外へ移した。EDIT モーダルの ⤓ LOAD はこれを叩く
- スマホでの EDIT への入口は既存のまま：音が入っているパッド＝窓の SAMPLE ページ → ✂、空パッド＝中央の ＋
- 失うもの：‹ › の前後移動（パッドを叩けば同じ）、窓の外の名前表示（パッドの枠と名前で分かる）
- PC は従来どおり strip を残す

検査：`check.mjs` 6d に「スマホは strip が none／390×844 でパッド 100px 以上／EDIT モーダルに LOAD と SMPL が出る」を追加。

教訓：**「移す」前に「もう在るか」を見る。** 既にある入口で足りるなら、消すだけの方が小さく済む。

## 137. タップテンポはメニューの TAP ボタンをやめ、窓の BPM カードを叩く（v0.3.126）
「タップテンポはテンポ設定カードを叩くで代行」。メニュー（⋮）の TAP ボタンを撤去し、窓の上段 **BPM カードを叩く＝タップテンポ**。

- 時刻は**押した瞬間**に取る（`tapTempoDown`）。適用は**離した時**（`tapTempoUp`）＝ドラッグした押下はタップから外す（`tapTempoCancel`）。
  タイミングの精度を落とさず、「叩いたつもりが数値をずらした」も「ずらしたつもりがテンポが飛んだ」も起きない
- 直近4タップの平均、2秒空いたらリセット、60〜180 に丸め（従来どおり）。`applyBpm` を通すので再生中の位相も保つ
- BPM カードは**2連打リセット（→100）の対象から外した**（タッチの2連打も、マウスの dblclick も）。叩けば叩くほどテンポになる
- 叩いた瞬間に数字が琥珀色に点く（`.tap-hit`）。窓の文言：`TAP → 120.0`

検査：`check.mjs` 5 に「500ms×4 で約120／2連打で100に戻らない／`#tap` が無い」。

教訓：**同じ物を触る操作は同じ場所に。** テンポの数値を触る場所と、テンポを叩く場所が別だったのは、メニューを開く1手の無駄だった。

## 138. START / END の数値欄を撤去（v0.3.126）
「スタートエンドの数値の表示は機能ごと要らない。数値は裏側で使うだけ」。

SAMPLE ページの START / END 欄（横ドラッグの数値）を撤去。値（`t.start` / `t.end`）は**波形のハンドル**（緑=STRT・赤=END）と
TRIM / CHOP が書き、再生・保存で裏で使うだけ。SAMPLE ページは **LOOP ＋ ✂ TRIM / CHOP** になった。

- `CL_SPEC` の start / end を撤去、`CL_PAGES.smpl.f = ["loop"]`
- 数値欄→ハンドルの同期コードは不要になった（ハンドル→値は従来どおり）
- Issue #145 の元案「START/END の数値欄に LOAD/SMPL を置く」は §136 で不要になり、欄そのものも消えた

検査：`check.mjs` 6 の「ハンドル→数値欄」「数値欄→ハンドル」を「ハンドル→`t.start` が動く／数値欄が無い」に差し替え。

## 139. スマホの ✂ が小さ過ぎた（v0.3.127）
「ハサミマークが小さ過ぎで NG」。スマホでは `#clWave` を「✂」1文字（13px）に縮めていた（START/END/LOOP と並べる幅が無かった名残）。
§138 で START/END の数値欄が消えて SAMPLE ページは LOOP ＋ ✂ だけになったので、PC と同じ **「✂ TRIM / CHOP」** を出し、字を 13px にした（幅 約140px）。

最初は高さも 44px にしたが、**SAMPLE ページだけ窓が 10px 高くなり、不変性の検査（§94：ページを移しても窓・パッドの寸法が動かない）で 20件落ちた**。
高さは他の欄と同じ 34px に戻し、指の当たり判定だけ `::after` で上下 5px ずつ広げて 44px 相当にした（見た目と寸法は動かさず、押しやすさだけ足す）。
あわせて deadcss が挙げた START/END 用の CSS（`[data-p="start"] u` の非表示）を1件撤去。

検査：`check.mjs` 6d に「SAMPLE ページの ✂ は幅広表記・幅 100px 以上・34px 高」。不変性の検査はそのまま番人。

教訓：**大きくするなら「幅」を使う。** 高さは窓の予約に縛られている。当たり判定は見た目と別に広げられる。

**v0.3.128 追記**：「文字で EDIT って入れれないの？」→ ボタンの文言を **EDIT** に（PC・スマホとも。✂ と TRIM / CHOP をやめる）。
開くモーダルの見出しも **EDIT** に揃えた（ボタンと窓の名前が同じ＝迷わない）。`.cl-narrow` の CSS は不要になり撤去。

## 140. 文字を選べなくした（v0.3.131）
「テキスト選択できないようにして」。パッドやラベルを長押し・ドラッグすると文字が青く選択され、楽器の見た目が崩れていた
（スマホの長押しコピー、PC の EDIT ドラッグで特に出る）。`body{user-select:none}` を全体に掛け、
**入力欄（`input` `textarea`：名前・合言葉）だけ `user-select:text`** に戻した。個別に付けてあった `user-select:none`
（`.pad` `.msbtn` 等）はそのまま（重複しても害はなく、消すと依存が分かりにくい）。

検査：`check.mjs` `copyPaint` に「本体・パッド・SEQ 行名・窓は選べない／入力欄は選べる」。

## 141. LOAD したパッドの SEQ 行名が古いままだった（v0.3.132）
Phase 2 第3段でパッド名の描画を `paintPadName` 1つに寄せたら見つかった。LOAD（`#samp`）と録音はパッド内の名前だけ書き換え、
SEQ 画面の行名（`.rn`）とステップ列の波形キャッシュは古いままだった（EDIT の改名や .mics 読込は両方書いていた）。
名前を書く口を1つにしたので、以後は出どころの違いで漏れない。

検査：`check.mjs` `copyPaint` に「LOAD で名前・パッド表示・SEQ 行名が揃う」「MOVE の undo で表示も戻る」。
