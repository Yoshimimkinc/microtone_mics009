...
## 132. P-LOCK を解除すると作り込んだ値が消える（v0.3.119）
「エフェクトP-LOCKを解除してもう一度適用すると数値はデフォルトに戻る。さっきの設定をキープしてほしい」。

再現（Playwright・1280×800 の Performance Mode）：

| 操作 | v0.3.118 | スマホ（同じ操作） |
|---|---|---|
| DELAY を選ぶ | send 0 | 0 |
| パッドを右へドラッグ | 0.70 | 0.70 |
| **DELAY を解除** | **0（巻き戻る）** | 0.70（残る） |
| もう一度 DELAY | 0 | 0.70 |

原因：`perfLockApply` が解放時に `perfRevertSnap()` で選択時のスナップショットへ戻していた。
Digitakt 式の「押している間だけ効く瞬間モジュレート」の思想で、保持したければ ● REC を点ける設計だった。
だが (1) スマホの DELAY/REVERB は `perfLockApply` を呼ばないので巻き戻らず、**同じ操作なのに PC と挙動が違った**、
(2) 送り量やフィルターは「演奏で一瞬ひねる」値である前に「作り込む」値で、消えると作業that が失われる。

直し方：**解放しても巻き戻さない**（`perfRevertSnap` を撤去）。戻すのは**パッドのダブルタップ**に一本化した
（基準＝そのP-LOCKを選んだ時の値）。● REC の意味は「再生中いじりをステップへ記録するか」だけになり、
値が残るかどうかとは無関係になった。検査は `check.mjs` §16（PC・スマホの両方で解除後も再適用後も 0.7 のまま）。

教訓：**同じ操作が画面によって違う結果になったら、まずどちらが正しいかを決める。** 片方だけ直すと食い違いが残る。

## 133. サンプル名を変える入口（v0.3.120）
「サンプル名を変更する入り口がどこにもない。EDITモードで1箇所、TRIM/CHOP の下に名前を表示し、横の編集ボタンで箱が出て編集」。
名前は録音（TAKE n）・読込（ファイル名）・CHOP（元名＋番号）で自動的に付くだけで、人が付け直す経路が無かった。

置き場所：EDIT モーダルの見出し `TRIM · CHOP` の**直下**に `01 PHRASE 1  [✎]`。v0.3.82 で「名前は背後に出ているからここには出さない」
と決めたが、**変える入口が要る以上、表示もここに要る**（見えない物は変えられない）。入口はここ1つ＝単一の真。

- ✎ を押すと入力箱が出る（16px＝iOS が拡大しない。合言葉の入力欄と同じ理由）
- Enter／欄外タップ＝確定、Escape＝取消。**Escape でモーダルごと閉じない**（入力箱で止める）
- 空欄・同じ名前は無視（Undo も積まない）。16文字まで。パッド内は従来どおり8文字・大文字

**表示経路の一本化**：名前は `tracks[i].name` と `PADS[i].name` の2系統に分かれていた
（`#edWho` や `stepTrackName` はサンプル以外だと `PADS[i].name` を見る）。`renameTrack()` が両方を書き、
読込時にも `PADS[i].name=t.name` で揃えるようにした。これをしないと SEQ の行ラベルがリロード後に既定へ戻る。

検査：`check.mjs` §17（表示・箱の開閉・Escape 取消・確定後のパッド/SEQ行/窓の追従・空欄無視）。
検査で1つ踏んだ：モーダルは開いた直後 450ms のクリックを捨てる（`peOpenedAt`）。検査の待ちも 600ms にした。
